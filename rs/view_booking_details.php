...
$stmt->execute();
$stmt->bind_result($cust_address_id, $b_cust_id, $booking_date, $booking_from_time, $booking_to_time, $bp_total,
...
$stmt = $conn->prepare("
    ...
    WHERE sind_id = ?
");
$stmt->bind_param("i", $_SESSION['sind_id']);
$stmt->execute();
...
<head>
    ...
    <title>Booking Details- Sinderella</title>
...
<body>
        ...
        <div class="content-container">
            ...
            <div class="details-container" style="display: flex;">
                ...
                <?php if ($booking_status == 'paid'): ?>
                    <button type="button" id="approve-btn" onclick="handleBookingAction('confirm')">Accept</button>
                    <!-- <button type="button" onclick="handleBookingAction('rejected')">Reject</button> -->
                     <button type="button" id="reject-btn" onclick="openRejectModal('rejected')">Reject</button>
                <?php endif; ?>
                ...
                <div id="rejectModal" style="display:none; position:fixed; top:0; left:0; width:100vw; height:100vh; background:rgba(0,0,0,0.7); z-index:9999; align-items:center; justify-content:center;">
                    <div style="background:#fff; padding:30px; border-radius:8px; text-align:center; min-width:320px; max-width:95vw;">
                        ...
                        <p style="margin-bottom:10px;">You have marked you are available on this time,<br> we would like to know why you want to reject the service?</p>
    ...
    <script>
        ...
        function submitRejectReason() {
            ...
            const bookingId = <?php echo json_encode($booking_id); ?>;
            const sindId = <?php echo json_encode($_SESSION['sind_id']); ?>;
            const rejectedThisMonth = <?php echo json_encode($rejected_this_month); ?>;
