...
<body>
    <div class="login-container">
        <div class="login-left">
            ...
            <p style="font-size:1rem;"><a href="../login_cust.php">< Back to Login Page</a></p>
        </div>
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $stmt = $conn->prepare("UPDATE customers SET cust_pwd = ? WHERE cust_phno = ?");
        if (!$stmt) {
            echo "<script>document.getElementById('error-message').innerText = 'Database error: " . $conn->error . "';</script>";
            exit();
        }
        ...
        echo "<script>alert('Password changed successfully.'); window.location.href = '../login_cust.php';</script>";
    }
    ?>
</body>
</html>