...
<body>
    <div class="login-container">
        <div class="login-left">
            ...
            <p style="font-size:1rem;"><a href="../login_adm.php">< Back to Login Page</a></p>
        </div>
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $stmt = $conn->prepare("SELECT adm_id FROM admins WHERE adm_phno = ?");
        if (!$stmt) {
        ...
        $stmt = $conn->prepare("UPDATE admins SET adm_pwd = ? WHERE adm_phno = ?");
        if (!$stmt) {
            echo "<script>document.getElementById('error-message').innerText = 'Database error: " . $conn->error . "';</script>";
            exit();
        }
        ...
        echo "<script>alert('Password changed successfully.'); window.location.href = '../login_adm.php';</script>";
    }
    ?>
</body>
</html>